    ...
    public function myOrderStore(OrderRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $oldOrder     = Order::where(['user_id' => $userId, 'active' => Status::INACTIVE, 'remember_token' => $session_id])->orderBy('id', 'desc');
                $orderReplace = $oldOrder;
                if (!blank($oldOrder->first())) {
                    ...
                    $stockReplace?->delete();
                    OrderAddress::whereIn('order_id', $ids)->where(['user_id' => $userId])?->delete();
                    OrderOutletAddress::whereIn('order_id', $ids)->where(['user_id' => $userId])?->delete();
                    OrderCoupon::whereIn('order_id', $ids)->where(['user_id' => $userId])?->delete();
                    $orderReplace->delete();
                }
                ...
                $this->order = Order::create(
                    $request->validated() + [
                        ...
                        'balance'        => number_format($request->balance, env('CURRENCY_DECIMAL_POINT'), '.', ''),
                    ]
                );
                ...
                $this->order->remember_token = $session_id;
                $this->order->save();
            ...
            }, 4); // Повторить три раза, прежде чем признать неудачу
            return $this->order;
        } catch (Exception $exception) {
            DB::rollBack();
            Log::info($exception->getMessage());
