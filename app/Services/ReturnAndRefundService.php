...
class ReturnAndRefundService
{
            ...
            if ($returnAndRefund) {
                ...
                DB::transaction(function () use ($request, $order) {
                    $this->returnAndRefund = ReturnAndRefund::create([
                        ...
                        'status'           => ($request->status ? $request->status : ReturnOrderStatus::PENDING),
                    ]);
    ...
    {
        try {
            $returnAndRefund = ReturnAndRefund::where('order_id', $request->orderId)->first();
            return $returnAndRefund;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
