...
class ProductVariationService
{
    ...
    public function initialVariation(Product $product)
    {
        try {
            ...
            if (count($variations)) {
                $variations = $product->variations()
                    ->with(['productAttribute:id,name,name_et,name_ru', 'productAttributeOption:id,name,name_et,name_ru', 'product:id,offer_start_date,offer_end_date,discount,show_stock_out,can_purchasable'])
                    ->get();
    ...
    {
        try {
            return $productVariation->children()->with(['productAttribute:id,name,name_et,name_ru', 'productAttributeOption:id,name,name_et,name_ru', 'product:id,offer_start_date,offer_end_date,discount,show_stock_out,can_purchasable'])->withSum('stockItems', 'quantity')->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
