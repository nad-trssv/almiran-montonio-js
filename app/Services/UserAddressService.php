...
class UserAddressService
{
     ...
     */
    public $address;
    public $addressFilter = ['full_name','email','phone', 'address', 'state','city','zip_code','latitude', 'longitude', 'web_token'];
    ...
    public function store($request, User $user): Address
    {
        try {
            DB::transaction(function () use ($request, $user) {
                $this->address = Address::create($request->validated() + ['user_id' => $user->id] + ['web_token' =>  $request->cookie('shopking_ecommerce_app_with_laravel_website_admin_panel_with_pos_inventory_management_session')]);
            });
            return $this->address;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
