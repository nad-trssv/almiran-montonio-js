...
class ProductService
{
    ...
    public function store(ProductRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                $storeName = $request->sku.'-'.$request->name;
                $this->product = Product::create($request->validated() + ['slug' => Str::slug($storeName), 'variation_price' => $request->selling_price]);
                if ($request->tags) {
    ...
    public function update(ProductRequest $request, Product $product): object
    {
        try {
            DB::transaction(function () use ($request, $product) {
                $storeName = $request->sku.'-'.$request->name;
                $product->update($request->validated() + ['slug' => Str::slug($storeName)]);
    ...
    public function mostPopularProducts(PaginateRequest $request)
    {
        try {
            ...
            return Product::select('products.id', 'products.name', 'products.sku', 'products.slug', 'products.selling_price', 'products.variation_price', 'products.add_to_flash_sale', 'products.offer_start_date', 'products.offer_end_date', 'products.discount', 'products.status')
                ...
                ->withReviewRating()
                ->withSum('orderCountable', 'quantity')
    ...
    public function categoryWiseProducts(Request $request): \Vanilla\Support\Collection|\IlluminateAgnostic\Str\Support\Collection|\IlluminateAgnostic\Collection\Support\Collection|\IlluminateAgnostic\StrAgnostic\Str\Support\Collection|\IlluminateAgnostic\ArrAgnostic\Arr\Support\Collection|\Illuminate\Support\Collection|\IlluminateAgnostic\Arr\Support\Collection
    {
        try {
            ...
            $productCategory = Product::select('products.id', 'products.name', 'products.name_et', 'products.name_ru', 'products.sku', 'products.slug', 'products.status', 'products.product_category_id', 'products.product_brand_id', 'products.variation_price', 'products.selling_price', 'products.discount')->with('brand', 'variations')
            ...
            $products = Product::select('products.id', 'products.name', 'products.name_et', 'products.name_ru', 'products.sku', 'products.slug', 'products.product_category_id', 'products.product_brand_id', 'products.selling_price', 'products.variation_price', 'products.add_to_flash_sale', 'products.offer_start_date', 'products.offer_end_date', 'products.discount', 'products.status')
                ...
                ->orderBy($orderColumn, $orderType)->where(function ($query) use ($request) {
                    if ($request->min_price >= 0 && $request->max_price > 0) {
                        $query->whereRaw('(variation_price - (variation_price / 100 * discount)) <= '.$request->max_price)->whereRaw('(variation_price - (variation_price / 100 * discount)) >= '.$request->min_price);
                    }
                })->paginate($perPage);
            ...
            $productAttributeOption_et = ProductAttributeOption::get()->pluck('name_et', 'id')->toArray();
            $productAttributeOption_ru = ProductAttributeOption::get()->pluck('name_ru', 'id')->toArray();
            if ($variations) {
                            ...
                            if (isset($variationArray[Str::slug($v['product_attribute']['name'], '_')])) {
                                ...
                                if ($status) {
                                    $variationArray[Str::slug($v['product_attribute']['name'], '_')][] = [
                                        ...
                                        'attribute_name_et'              => $v['product_attribute']['name_et'],
                                        'attribute_name_ru'              => $v['product_attribute']['name_ru'],
                                        ...
                                        'attribute_option_name_et'       => isset($productAttributeOption_et[$v['product_attribute_option_id']]) ? $productAttributeOption_et[$v['product_attribute_option_id']] : '',
                                        'attribute_option_name_ru'       => isset($productAttributeOption_ru[$v['product_attribute_option_id']]) ? $productAttributeOption_ru[$v['product_attribute_option_id']] : '',
    ...
    public function flashSaleProducts(PaginateRequest $request)
    {
        try {
            ...
            return Product::select('products.id', 'products.name', 'products.name_et', 'products.name_ru', 'products.sku', 'products.slug', 'products.selling_price', 'products.variation_price', 'products.add_to_flash_sale', 'products.offer_start_date', 'products.offer_end_date', 'products.discount', 'products.status')
                ->withReviewRating()
                ->with(['wishlist' => fn ($query) => $query->where('user_id', Auth::check() ? Auth::user()->id : 0)])
    ...
    {
        try {
            return Product::select('id', 'name', 'name_et', 'name_ru', 'buying_price', 'can_purchasable', 'status', 'sku')
                ->with('productTaxes')
                ->with('variations')
    ...
    {
        try {
            return Product::select('id', 'name', 'name_et', 'name_ru', 'buying_price', 'status', 'sku')
                ->with('productTaxes')
                ->with('variations')
                ->orderBy('name', 'asc')
                ->get();
    ...
    public function ancestorCategoryWiseProducts(ProductCategory $category, $rand = null)
    {
        ...
        try {
            ...
            return Product::select('id', 'name', 'name_et', 'name_ru', 'sku', 'slug', 'status', 'product_category_id')
