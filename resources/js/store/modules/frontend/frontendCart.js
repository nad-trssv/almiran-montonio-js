...
export const frontendCart = {
    ...
    getters: {
        ...
        outletUserName: function (state) {
            return state.outletUserName;
        },
        outletUserPhone: function (state) {
            return state.outletUserPhone;
        },
        outletUserEmail: function (state) {
            return state.outletUserEmail;
        },
        ...
        usedBalance: function (state) {
            return state.usedBalance;
        }
    },
    actions: {
        useBalance: function (context, payload) {
            context.commit("balance", payload.balance);
            context.commit("usedBalance", payload.usedBalance);
        ...
        outletUserName: function (context, payload) {
            context.commit('outletUserName', payload);
        },
        outletUserPhone: function (context, payload) {
            context.commit('outletUserPhone', payload);
        },
        outletUserEmail: function (context, payload) {
            context.commit('outletUserEmail', payload);
        },
    ...
    mutations: {
        subtotal: function (state) {
            ...
            if (state.lists.length > 0) {
                ...
                _.forEach(state.lists, (list, listKey) => {
                    ...
                    state.lists[listKey].total    = ((state.lists[listKey].price * state.lists[listKey].quantity)) - state.lists[listKey].discount;
                    subtotal                      += state.lists[listKey].subtotal;
                    total                         += state.lists[listKey].total;
                });
                state.subtotal = subtotal;
                state.total    = total;
                if((state.subtotal - state.discount) >= 100){
        ...
        outletUserName: function (state, payload) {
            state.outletUserName = payload;
        },
        outletUserPhone: function (state, payload) {
            state.outletUserPhone = payload;
        },
        outletUserEmail: function (state, payload) {
            state.outletUserEmail = payload;
        },
            ...
            _.forEach(state.lists, (list, listKey) => {
                if (list.taxes.length > 0) {
                    ...
                    _.forEach(list.taxes, (tax, taxKey) => {
                        if (tax.tax_rate > 0) {
                            let taxPercentagePrice = ((list.price / 100));
                ...
                if (payload.setting.shipping_setup_method === shippingMethodEnum.FLAT_RATE_WISE) {
                    ...
                    if (Object.keys(state.shippingAddress).length > 0) {
                        ...
                        _.forEach(payload.area, (list, listKey) => {                        
                            if (list.country === state.shippingAddress.country) {
                                status               = true;
                        ...
                        if((state.subtotal - state.discount) >= 100) {
                            status               = true;
                            state.total = (state.total - state.shippingCharge);
                            state.shippingCharge = 0;
                        }
        ...
        usedBalance: function (state, payload) {
            state.usedBalance = payload;
        },
        resetCart: function (state) {
            ...
            state.outletAddress   = {};
            state.outletUserName   = '';
            state.outletUserPhone   = '';
            state.outletUserEmail   = '';
            state.paymentMethod   = {};
            state.totalTax        = 0;
            state.shippingCharge  = 0;
            state.balance = false;
