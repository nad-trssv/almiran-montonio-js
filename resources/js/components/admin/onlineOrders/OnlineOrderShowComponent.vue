<template>
    ...
    <div class="col-12">
                <div class="flex items-center gap-4 mb-7">
                    ...
                    <h2 class="capitalize text-xl font-bold text-primary">{{ $t('label.return') }}</h2>
                </div>
        <div class="db-card p-4">
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                ...
                <div class="flex flex-wrap gap-3"
                    ...
                    <router-link
                        v-if="returnId === 0"
                        :to="{ name: 'admin.returnAndRefund.request', params: { id: order.id } }"
                        class="flex items-center justify-center gap-2 px-4 h-[38px] rounded shadow-db-card bg-primary text-white">
                        {{ $t('button.return_request') }}
                    </router-link>

                    <router-link
                        v-else
                        :to="{ name: 'admin.returnAndRefund.show', params: { id: returnId } }"
                        class="flex items-center justify-center gap-2 px-4 h-[38px] rounded shadow-db-card bg-success text-white">
                        {{ $t('label.details') }}
                    </router-link>

                    <OnlineOrderReceiptComponent :order="order" :orderProducts="orderProducts" :orderUser="orderUser"
                        :orderAddress="orderAddress" v-if="orderUser"/>
                    <OnlineOrderReceiptComponent :order="order" :orderProducts="orderProducts"
                        :orderAddress="orderAddress" v-else/>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row">
                ...
                <div class="db-card">
                    ...
                    <div class="db-card-body">
                        <div class="flex items-center gap-3 mb-4">
                            <img class="w-8 rounded-full" alt="avatar" :src="orderUser.image" v-if="orderUser">
            ...
            <div class="col-12" v-else-if="order.order_type === enums.orderTypeEnum.PICK_UP">
                <div class="db-card">
                    ...
                    <div class="db-card-body">
                        <div class="flex items-center gap-3 mb-4" v-if="orderUser">
                            <img class="w-8 rounded-full" alt="avatar" :src="orderUser.image">
                            <h4 class="font-semibold text-sm capitalize text-[#374151]">
                                {{ textShortener(orderUser.name, 20) }}
                            </h4>
                        </div>
                        <div class="flex items-center gap-3 mb-4" v-if="outletAddress.name">
                            <h4 class="font-semibold text-sm capitalize text-[#374151]">
                                {{ textShortener(outletAddress.name, 20) }}
                            </h4>
                        </div>
                        <ul class="flex flex-col gap-3 py-4 border-t border-[#EFF0F6]">
                            <li v-if="orderUser" class="flex items-center gap-2.5">
                                <i class="lab lab-line-mail lab-font-size-14"></i>
                                <span class="text-xs">{{ orderUser.email }}</span>
                            </li>
                            <li v-if="outletAddress.email" class="flex items-center gap-2.5">
                                <i class="lab lab-line-mail lab-font-size-14"></i>
                                <span class="text-xs">{{ outletAddress.email }}</span>
                            </li>
                            <li v-if="orderUser" class="flex items-center gap-2.5">
                                <i class="lab lab-line-call-calling lab-font-size-14"></i>
                                <span class="text-xs">{{ orderUser.country_code + '' + orderUser.phone }}</span>
                            </li>
                            <li v-if="outletAddress.phone" class="flex items-center gap-2.5">
                                <i class="lab lab-line-call-calling lab-font-size-14"></i>
                                <span class="text-xs">{{ outletAddress.country_code + '' + outletAddress.phone }}</span>
                            </li>
                            <li class="flex items-center gap-2.5">
                                <i class="lab lab-line-location lab-font-size-14"></i>
                                <span class="text-xs">
...
export default {
    ...
    mounted() {
        ...
        this.loading.isActive = true;
        this.$store.dispatch('returnAndRefund/showByOrderId', this.$route.params.id).then(res => {
            this.payment_status = res.data.data.payment_status;
            this.order_status = res.data.data.status;
