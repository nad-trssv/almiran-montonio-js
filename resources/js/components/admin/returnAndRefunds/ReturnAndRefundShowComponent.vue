        ...
        <div class="db-card p-4">
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                ...
                <div class="flex flex-wrap gap-3" v-if="order.status === enums.returnStatusEnum.PENDING">
                    ...
                    <button type="button" @click="changeStatus(enums.returnStatusEnum.ACCEPT, false)"
                    ...
                    <button type="button" @click="changeStatus(enums.returnStatusEnum.ACCEPT, true)"
                        v-if="order.status === enums.returnStatusEnum.PENDING"
                        class="flex items-center justify-center text-white gap-2 px-4 h-[38px] rounded shadow-db-card bg-admin-blue">
                        <i class="lab lab-fill-save"></i>
                        <span class="text-sm text-white">{{ $t('button.accept_and_return_credit') }}</span>
                    </button>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        changeStatus: function (status, sendCredit) {
