  ...
  <section class="mb-10 sm:mb-20">
    <div class="container">
      <CategoryBreadcrumbComponent
        v-if="
          typeof $route.query.category !== 'undefined' &&
      ...
      <div class="flex items-center justify-between gap-5 mb-6 max-md:mb-8">
        <div
          class="flex flex-wrap items-end gap-3 max-md:flex-col max-md:items-start max-md:gap-1.5"
        >
          ...
          <span class="text-xl font-medium capitalize max-sm:text-sm">
            ...
            {{
              categoryWiseProducts.length > 1
                ? $t("label.products_found")
                : $t("label.product_found")
            }})
          </span>
        </div>
        <button
          @click.prevent="showTarget('filter-canvas', 'canvas-active')"
          type="button"
          class="lab-line-filter md:invisible flex-shrink-0 text-2xl w-9 h-9 leading-9 text-center rounded-full border border-primary text-primary md:hidden"
        ></button>
      </div>
        ...
        >
          ...
          >
            <div class="max-md:h-screen max-md:overflow-y-auto">
              ...
              >
                <h3 class="text-[22px] font-bold capitalize">
                  {{ $t("label.filter_and_sorting") }}
                </h3>
                <button
                  @click.prevent="hideTarget('filter-canvas', 'canvas-active')"
                  type="button"
              ...
              <div class="filter-group border-b border-gray-100">
                <button
                  :key="'eventSortBy'"
                  @click.prevent="colspanHideShow($event, 'sortBy')"
                  type="button"
                ...
                >
                  <span
                    ...
                    >{{ $t("label.sort_by") }}</span
                  >
                  <i
                    class="lab-line-chevron-up text-base font-semibold transition-all duration-500 group-hover:text-primary"
                  ></i>
                </button>
                <div
                  :key="'eventSortById'"
                ...
                >
                  <div class="w-full mb-6 flex flex-col gap-4 px-6 md:px-0">
                    ...
                    >
                      <input
                        @click="sortByOption($event, 'newest')"
                    ...
                    >
                      <input
                        @click="sortByOption($event, 'price_low_to_high')"
                        v-model="productSortBy"
                    ...
                    >
                      <input
                        @click="sortByOption($event, 'price_high_to_low')"
                        v-model="productSortBy"
                    ...
                    >
                      <input
                        @click="sortByOption($event, 'top_rated')"
                        v-model="productSortBy"
                        value="top_rated"
                        type="radio"
                        id="topRated"
              ...
              <div class="filter-group border-b border-gray-100">
                <button
                  :key="'eventFilterPrice'"
                  @click.prevent="colspanHideShow($event, 'filterPrice')"
                  type="button"
                  class="filter-btn active group flex items-center justify-between w-full py-5 px-4 ltr:md:pl-0 rtl:pr-0"
                >
                  <span
                ...
                >
                  <div class="w-full mb-6 flex flex-col gap-4 px-6 md:px-0">
                    ...
                    <VueSimpleRangeSlider
                      ...
                      active-bar-color="#F23E14"
                      bar-color="#D9DBE9"
                      class="p-1 w-full mobile:hidden"
                      :min="0"
                      :max="maxRange"
              ...
              <div
                v-if="Object.keys(categoryWiseVariations).length > 0"
                v-for="(
                  categoryWiseVariation, categoryWiseVariationKey
                ) in categoryWiseVariations"
              ...
              >
                <button
                  ...
                  @click.prevent="
                    colspanHideShow(
                      $event,
                      'variation_' + categoryWiseVariationKey
                    )
                  "
                  type="button"
                  class="filter-btn active group flex items-center justify-between w-full py-5 px-4 ltr:md:pl-0 rtl:pr-0"
                >
                  <span
                    ...
                    class="text-lg font-semibold capitalize transition-all duration-500 group-hover:text-primary"
                  >
                    {{ underscoreToSpace(categoryWiseVariation[0].attribute_name) }}
                  </span>
                  <span
                    v-else-if="language.id == 2 && categoryWiseVariation[0].attribute_name_et !== null && categoryWiseVariation[0].attribute_name_et !== ''"
                    class="text-lg font-semibold capitalize transition-all duration-500 group-hover:text-primary"
                  >
                    {{ underscoreToSpace(categoryWiseVariation[0].attribute_name_et) }}
                  </span>
                  <span
                    v-else-if="language.id == 3 && categoryWiseVariation[0].attribute_name_ru !== null && categoryWiseVariation[0].attribute_name_ru !== ''"
                    class="text-lg font-semibold capitalize transition-all duration-500 group-hover:text-primary"
                  >
                    {{ underscoreToSpace(categoryWiseVariation[0].attribute_name_ru) }}
                  </span>
                  <span
                    v-else
                    class="text-lg font-semibold capitalize transition-all duration-500 group-hover:text-primary"
                ...
                <div
                  :key="'variation_' + categoryWiseVariationKey + 'Id'"
                ...
                >
                  <div class="w-full mb-6 flex flex-col gap-4 px-6 md:px-0">
                    <label
                      :for="
                        variation.product_attribute_id +
                        '_' +
                        variation.product_attribute_option_id
                      "
                    ...
                    >
                      ...
                      <span
                        v-else-if="language.id == 2 && variation.attribute_option_name_et !== null && variation.attribute_option_name_et !== ''"
                        class="font-medium capitalize transition-all duration-500 group-hover:text-primary"
                      >
                        {{ variation.attribute_option_name_et }}
                      </span>
                      <span
                        v-else-if="language.id == 3 && variation.attribute_option_name_ru !== null && variation.attribute_option_name_ru !== ''"
                        class="font-medium capitalize transition-all duration-500 group-hover:text-primary"
                      >
                        {{ variation.attribute_option_name_ru }}
                      </span>
                      <span
                        v-else
                        class="font-medium capitalize transition-all duration-500 group-hover:text-primary"
        ...
        <div class="w-full p-4 max-md:p-0">
          <div class="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            ...
            <ProductListComponent
              v-if="categoryWiseProducts.length > 0"
...
export default {
  ...
  computed: {
    pagination: function () {
      return this.$store.getters[
        "frontendProduct/categoryWiseProductPagination"
      ];
    },
  ...
  methods: {
    ...
    ancestorsAndSelf: function () {
      ...
      if (
        typeof this.$route.query.brand !== "undefined" &&
        this.$route.query.brand !== ""
      ) {
        this.productSearchForm.brand = JSON.stringify([
          this.$route.query.brand,
        ]);
      } else {
      ...
      this.$store
        .dispatch(
          "frontendProduct/categoryWiseProducts",
    ...
    priceOptionUpdate() {
      this.productPrice.range = [
        this.productPrice.range[0],
        this.productPrice.range[1],
      ];
      this.productSearchForm.min_price = this.productPrice.range[0];
      this.productSearchForm.max_price = this.productPrice.range[1];
      this.products();
    ...
    variationOption: function (event, attribute, option) {
      if (event.target.checked) {
        ...
        for (let i = 0; i < this.productVariations.length; i++) {
          if (
            this.productVariations[i].attribute === attribute &&
            this.productVariations[i].option === option
          ) {
