    ...
    <div class="row">
        <div class="col-12 lg:col-8">
            ...
            <div v-if="orderType === orderTypeEnum.PICK_UP" class="mb-6 rounded-2xl shadow-card">
                ...
                <div class="form-row px-4 pt-4">
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <input type="text" @change="outletUserName($event)" v-model="userName" required
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        ...
                        <input type="text" @change="outletUserPhone($event)" v-model="userPhone" required
                            class="w-full h-12 px-4 rounded-lg text-base border border-[#D9DBE9] hover:border-primary/30 focus-within:border-primary/30 transition-all duration-500">
                    </div>
                    <div class="form-col-12">
                        <label for="email" class="text-sm font-medium capitalize mb-1 field-title required">
                            {{ $t('label.email') }}
                        </label>
                        <input type="text" @change="outletUserEmail($event)" v-model="userEmail" required
                            class="w-full h-12 px-4 rounded-lg text-base border border-[#D9DBE9] hover:border-primary/30 focus-within:border-primary/30 transition-all duration-500">
                    </div>
                </div>
            </div>
            ...
            <div class="max-lg:hidden flex items-center justify-between gap-5 mt-10">
                ...
                <button @click.prevent="selectAddress" :disabled="enableSelectAddressButton" type="button"
                    :class="enableSelectAddressButton === false ? 'shadow-btn-primary !bg-primary' : ''"
...
export default {
    ...
    data() {
        return {
            ...
            userPhone: '',
            userEmail: '',
            enableSelectAddressButton: false,
        }
    },
    computed: {
        setting: function () {
    ...
    methods: {
        changeOrderType: function (e) {
            ...
            if (this.orderType === orderTypeEnum.DELIVERY) {
                this.enableSelectAddressButton = false;   
            }
            if (this.orderType === orderTypeEnum.PICK_UP) {
                this.enableSelectAddressButton = true;   
            }
        },
        ...
        outletAddress: function(e) {
            ...
            this.checkSaveAndPayBtn();
        },
        outletUserName: function(e) {
            setTimeout(() => {
                this.$store.dispatch('frontendCart/outletUserName', this.userName).then().catch();
            }, 100);
            this.checkSaveAndPayBtn();
        },
        outletUserPhone: function(e) {
            setTimeout(() => {
                this.$store.dispatch('frontendCart/outletUserPhone', this.userPhone).then().catch();
            }, 100);
            this.checkSaveAndPayBtn();
        },
        outletUserEmail: function(e) {
            setTimeout(() => {
                this.$store.dispatch('frontendCart/outletUserEmail', this.userEmail).then().catch();
            }, 100);
            this.checkSaveAndPayBtn();
        },
        checkSaveAndPayBtn: function() {
            if (this.orderType === orderTypeEnum.PICK_UP) {
                if(this.userName !== '' && this.userName !== '' && this.userPhone !== '' && this.modelOutlet !== 0) {
                    this.enableSelectAddressButton = false;   
                } else {
                    this.enableSelectAddressButton = true;
                }
