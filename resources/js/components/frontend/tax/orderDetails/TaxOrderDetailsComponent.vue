<template>
            ...
            <div class="col-12 md:col-7">
                <div class="rounded-2xl border border-gray-100">
                    ...
                    <ul class="flex flex-col gap-2 py-4 mx-4 border-b border-dashed border-[#EFF0F6]">
                        ...
                        <li class="flex items-center justify-between" v-if="order.balance > 0">
                            <span class="capitalize">{{ $t('label.balance') }}</span>
                            <span class="font-medium text-[#FB4E4E]">-{{ currencyFormat(order.balance, setting.site_digit_after_decimal_point,
    ...
    <div v-if="order.status !== enums.orderStatusEnum.CANCELED" class="flex flex-wrap gap-6 mobile:mb-20">
        <OrderReceiptComponent :order="order" :orderProducts="orderProducts"
            :orderAddress="orderAddress[0]" :walletBalance="currencyFormat(order.balance, setting.site_digit_after_decimal_point,
                        setting.site_default_currency_symbol, setting.site_currency_position)" />
        <button v-if="order.status === enums.orderStatusEnum.PENDING && isAuth === true" @click="changeStatus(enums.orderStatusEnum.CANCELED)"
            type="button"
        ...
        <router-link v-if="order.status === enums.orderStatusEnum.DELIVERED && order.return_and_refund && isAuth === true"
...
import OrderReceiptComponent from "./TaxOrderReceiptComponent";
import orderTypeEnum from "../../../../enums/modules/orderTypeEnum";
import ENV from "../../../../config/env";

export default {
    name: "TaxOrderDetailsComponent",
    ...
    mounted() {
        if (this.$route.params.id) {
            ...
            this.$store.dispatch("frontendOrder/show", this.$route.params.id).then(res => {
                ...
                this.loading.isActive = false;
                window.location.href = ENV.API_URL + "/home";
            });
        }
        (this.profile.id != null)?this.isAuth=true:false;
    },
    methods: {
